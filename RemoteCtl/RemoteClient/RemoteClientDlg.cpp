...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_IPAddress(pDX, IDC_IPADDRESS_Serv, m_server_adress);
...
int CRemoteClientDlg::sendCommandPacket(int nCmd, bool bAutoClose, BYTE* pData, size_t length)
{
	...
	bool ret = pClient->InitSocket(m_server_adress, atoi((LPCTSTR)m_nPort));//todo 返回值处理
	if (!ret) {
...
void CRemoteClientDlg::OnBnClickedBtnFileinfo()
{
	...
	CClientSocket* pClient = CClientSocket::getInstance();
	std::string drivers = pClient->GetPack().strData;
	std::string dr;
...
void CRemoteClientDlg::OnNMDblclkTreeDir(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	sendCommandPacket(2,false, (BYTE*)(LPCSTR) strPath,strPath.GetLength());
	...
	while (pInfo->HasNext){
		if (pInfo->IsDirectory) {
			if (CString(pInfo->szFilename) == "." || CString(pInfo->szFilename) == "..") {
				...
				TRACE("ack: %d\r\n", cmd);
				if (cmd < 0)break;
				pInfo = (PFILEINFO)CClientSocket::getInstance()->GetPack().Data();
				continue;
			}
		}
