...
//返回值是命令号，如果小于0，则是错误
	...
	int GetImage(CImage& image) {
		CClientSocket* pClient = CClientSocket::getInstance();
		return CPracticeTool::Byte2Image(image, pClient->GetPack().strData.c_str());
	}
...
protected:
	...
	LRESULT OnSendPack(UINT
		nMsg, WPARAM wParam, LPARAM lParam);

	LRESULT OnSendData(UINT
		nMsg, WPARAM wParam, LPARAM lParam);

	LRESULT OnShowStatus(UINT
		nMsg, WPARAM wParam, LPARAM lParam);

	LRESULT OnShowWatcher(UINT
		nMsg, WPARAM wParam, LPARAM lParam);
